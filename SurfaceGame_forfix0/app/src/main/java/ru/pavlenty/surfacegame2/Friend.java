...
d=false;
    ...
    public void update(int speed) {
        ...
        if (x < 0 || d) {
            d=false;
            if(x<0) GameView.Ah();
            bitmap=BitmapFactory.decodeResource(gg.getResources(), R.drawable.boom);
            x =(int)(Math.random()*2000)+3500;
            Random generator = new Random();
            y = generator.nextInt(maxY);
            bitmap=BitmapFactory.decodeResource(gg.getResources(), R.drawable.friend);
    ...
    public void destroyed() {
        bitmap=BitmapFactory.decodeResource(gg.getResources(), R.drawable.boom);
        gg.GetCanvas().drawBitmap(bitmap,
                x, y,
                paint);
        try {
            Thread.sleep(50);
